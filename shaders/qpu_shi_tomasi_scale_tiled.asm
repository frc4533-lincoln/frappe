...
mov     tgt_addr,       unif;
mov	    stride,         unif;
# row_start and row_incr are floating point numbers
mov	    row_start,      unif;
mov	    row_incr,       unif;
mov	    line_count,     unif;
mov     ofs_addr,       unif;
...
mul24   r0,         elem_num,       4
add     ofs_addr,   ofs_addr,       r0
# add     src_addr,   src_addr,       r0
nop
# -------------------------------------------------------------
# Assume we get a stream of uniforms that specify the address offsets
#
# The maximum offset we need is for scale down 64/0.25 = 256 pixels
# We can extract from byte lanes into 32 bit signed with the .8[abcd]si unpack mode
# 
# We need four uniforms to specify each offset register = 20 uniforms, and a lot
# of lane manipulation. Is it easier and faster to just load the offset data directly?
#
# With direct load, we need 32 words
mov     r0,         ofs_addr
mov     t0s,        r0;                     add     r0,         r0,         num64
mov     t0s,        r0;                     add     r0,         r0,         num64
mov     t0s,        r0;                     add     r0,         r0,         num64
mov     t0s,        r0;                     add     r0,         r0,         num64
mov     t0s,        r0
ldtmu0
mov     t0s,        r0
mov     offset0,    r4;                     ldtmu0
mov     offset1,    r4;                     ldtmu0
mov     offset2,    r4;                     ldtmu0
mov     offset3,    r4;                     ldtmu0
mov     offset4,    r4;                     ldtmu0
...
#
# Floating point row address generation
# src_ptr = int(row_start) * stride + src_addr
# row_start = row_start + row_incr



# Fetch first two rows
ftoi    r0,         row_start;              mov     r1,         row_start
mul24   r0,         r0,         stride;     fadd    r1,         r1,         row_incr
add     r0,         src_addr,   r0;         mov     row_start,  r1
