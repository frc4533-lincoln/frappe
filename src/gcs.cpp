...
#define CHECK_STATUS_M(STATUS, MSG, ERRHANDLER) \
	if (STATUS != MMAL_SUCCESS) { \
		printf("%s: %s\n", MSG, mmal_status_to_string(STATUS)); \
		goto ERRHANDLER; \
	}
...
GCS *gcs_create(GCS_CameraParams *cameraParams)
{
    ...
    // Move definition here to fix goto compile error in c++
	...
	GCS *gcs = (GCS*)vcos_calloc(1, sizeof(*gcs), "gcs");
	...
	format = gcs->cameraOutput->format;
