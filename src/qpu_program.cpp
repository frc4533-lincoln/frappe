...
int qpu_initProgram(QPU_PROGRAM *program, QPU_BASE *base, QPU_PROGMEM progmem)
{
	...
	uint32_t progmemSize = progmem.codeSize + progmem.uniformsSize * 4 + progmem.messageSize * 4 + progmem.constantsSize * 4;
	...
	program->progmem.uniforms.arm.vptr  = (char*)pmstart.arm.vptr + progmem.codeSize;
	...
	program->progmem.message.arm.vptr   = (char*)pmstart.arm.vptr + progmem.codeSize + progmem.uniformsSize*4;
	program->progmem.messageSize        = progmem.messageSize;
	program->progmem.constants.vc       = pmstart.vc + progmem.codeSize + progmem.uniformsSize * 4 + progmem.messageSize * 4;
	program->progmem.constants.arm.vptr = (char*)pmstart.arm.vptr + progmem.codeSize + progmem.uniformsSize * 4 + progmem.messageSize * 4;
	program->progmem.constantsSize      = progmem.constantsSize;
...
int qpu_executeProgramDirect (QPU_PROGRAM *program, QPU_BASE *base, int numInst, int unifLength, int unifStride, QPU_PerformanceState *perfState)
{
    ...
    // Enable all qpus for everything
    base->peripherals[V3D_SQRSV0] = 0;//0x10001000;
    base->peripherals[V3D_SQRSV1] = 0;//0x00001000;

	// Clear caches - L2, TMU, uniforms, instructions
	base->peripherals[V3D_L2CACTL] = (1<<2); // Clear L2 cache
    // Clear icache, uniform cache, tmu0, tmu1 cache
	...
	//base->peripherals[V3D_SRQCS] = (1<<7) | (1<<8) | (1<<16); // Reset error bit and counts



    mvprintw(85, 60, "QPU instances: %4d  queued: %4d finished: %4d   ", numInst, qpuQueued, qpuFinished);
	...
	for (int q = 0; q < numInst; q++)
	{
		...
		while((base->peripherals[V3D_SRQCS] & 0b111111) == 16)
		{
			...
			if (cnt % 1000 == 0 && perfState != NULL)
            {
                qpu_logPerformance(perfState);
            }
			if (cnt % 100000 == 0)
			{
				mvprintw(86, 60, "QPU stalled - queued %d / %d! \n", q, numInst);
				return -1;
			}
		}
...
unsigned int qpu_getCodeSize(const char *filename)
{
    ...
    if (!file)
    {
        printf("Failed to load QPU program %s!\n", filename);
        exit(1);
    }
	fseek(file, 0, SEEK_END);
	unsigned int fileLength = ftell(file);
