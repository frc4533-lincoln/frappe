...
void qpu_logPerformance (QPU_PerformanceState *state)
{
	...
	mvprintw(88, 60, "     Cycles: %llu idle | %llu vert | %llu frag | %llu instructions\n", diff.clkIdle, diff.clkVert, diff.clkFrag, diff.clkInst);
	mvprintw(89, 60, "     Clock %uMHz - %uMHz | %.01f%% total load | %.01f%% load for used QPUs | %.1f°C SoC temp\n", state->clockRateMin/1000000, state->clockRateMax/1000000, qpuLoadPerc, qpuUsedLoadPerc, state->tempSOC/1000.0f);
	mvprintw(90, 60, "     %llu instructions | %llu load cycles | %.01f%% stalls\n", diff.clkInst, loadCycles, qpuStallPerc);
	mvprintw(91, 60, "     TMU: %.1f avg access interval | %.1f avg stall cycles | %.1f%% cached\n", TMUUsageInter, avgTMUStalls, TMUCachedPerc);
	mvprintw(92, 60, "     VPM: %.1f%% VDW stall | %.1f%% VCD stall (avg of program cycles)\n", VDWStallPerc, VCDStallPerc);
	mvprintw(93, 60, "     L2C: %.2f%% miss (%llu hits | %llu misses)\n", L2CMissPerc, diff.numL2CHits, diff.numL2CMiss);
...
int qpu_logErrors(QPU_BASE *base)
{
	...
	if (errstat & (1<<15)) mvprintw(86, 60, "L2CARE: L2C AXI Receive Fifo Overrun error!\n");
	if (errstat & (1<<14)) mvprintw(86, 60, "VCMBE: VCM error (binner)!\n");
	if (errstat & (1<<13)) mvprintw(86, 60, "VCMRE: VCM error (renderer)!\n");
	// 12: IDLE
	if (errstat & (1<<11)) mvprintw(86, 60, "VCDE: VCD error - FIFO pointers out of sync!\n");
	if (errstat & (1<<10)) mvprintw(86, 60, "VDWE: VDW error - address overflows!\n");
	if (errstat & (1<< 9)) mvprintw(86, 60, "VPMEAS: VPM error - allocated size error!\n");
	if (errstat & (1<< 8)) mvprintw(86, 60, "VPMEFNA: VPM error - free non-allocated!\n");
	if (errstat & (1<< 7)) mvprintw(86, 60, "VPMEWNA: VPM error - write non-allocated!\n");
	if (errstat & (1<< 6)) mvprintw(86, 60, "VPMERNA: VPM error - read non-allocated!\n");
	if (errstat & (1<< 5)) mvprintw(86, 60, "VPMERR: VPM error - read range!\n");
	if (errstat & (1<< 4)) mvprintw(86, 60, "VPMEWR: VPM error - write range!\n");
	if (errstat & (1<< 3)) mvprintw(86, 60, "VPAERRGL: VPM Allocator error - renderer request greater than limit!\n");
	if (errstat & (1<< 2)) mvprintw(86, 60, "VPAEBRGL: VPM Allocator error - binner request greater than limit!\n");
	if (errstat & (1<< 1)) mvprintw(86, 60, "VPAERGS: VPM Allocator error - request too big!\n");
	if (errstat & (1<< 0)) mvprintw(86, 60, "VPAEABB: VPM Allocator error - allocating base while busy!\n");

	// PSE Error Signals
	hadError |= errstat & (0x1F06);
	if (base->peripherals[V3D_DBGE] & (1<<20)) mvprintw(86, 60, "IPD2_FPDUSED: error_ipd2_fpdused in PSE!\n");
	if (base->peripherals[V3D_DBGE] & (1<<19)) mvprintw(86, 60, "IPD2_VALID: error_ipd2_valid in PSE!\n");
	if (base->peripherals[V3D_DBGE] & (1<<18)) mvprintw(86, 60, "MULIP2: error_mulip2 in PSE!\n");
	if (base->peripherals[V3D_DBGE] & (1<<17)) mvprintw(86, 60, "MULIP1: error_mulip1 in PSE!\n");
	if (base->peripherals[V3D_DBGE] & (1<<16)) mvprintw(86, 60, "MULIP0: error_mulip0 in PSE!\n");
	if (base->peripherals[V3D_DBGE] & (1<< 2)) mvprintw(86, 60, "VR1_B: error b reading VPM in PSE!\n");
	if (base->peripherals[V3D_DBGE] & (1<< 1)) mvprintw(86, 60, "VR1_A: error a reading VPM in PSE!\n");
